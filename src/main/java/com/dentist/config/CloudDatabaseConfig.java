...
@Configuration
@EnableJpaRepositories(basePackages = "com.dentist.repository.cloud", entityManagerFactoryRef = "cloudEntityManagerFactory", transactionManagerRef = "cloudTransactionManager")
public class CloudDatabaseConfig {

	@Bean(name = "cloudEntityManagerFactory")
	LocalContainerEntityManagerFactoryBean cloudEntityManagerFactory(EntityManagerFactoryBuilder builder,
			@Qualifier("cloudDataSource") DataSource cloudDataSource) {
		return builder.dataSource(cloudDataSource).packages("com.dentist.beans").persistenceUnit("cloud").build();
	}
